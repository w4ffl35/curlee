                          ...
                          const std::unordered_map<std::string, std::string>& substitutions)
{
    return std::visit(
        [&](const auto& node) -> std::string
        {
            ...
            else if constexpr (std::is_same_v<T, curlee::parser::PredCall>)
            {
                std::string out = std::string(node.callee) + "(";
                for (std::size_t i = 0; i < node.args.size(); ++i)
                {
                    ...
                    out += pred_to_string_with_subst(node.args[i], substitutions);
                }
                out += ")";
                return out;
            }
                                         ...
                                         const curlee::parser::Expr* best)
{
    ...
    else if (const auto* phys_read =
                 std::get_if<curlee::parser::RuntimePhysReadExpr>(&expr.node))
    {
        if (phys_read->addr)
        {
            best = find_expr_at(*phys_read->addr, offset, best);
        }
    }
    else if (const auto* phys_write =
                 std::get_if<curlee::parser::RuntimePhysWriteExpr>(&expr.node))
    {
        if (phys_write->addr)
        {
            best = find_expr_at(*phys_write->addr, offset, best);
        }
        if (phys_write->value)
        {
            best = find_expr_at(*phys_write->value, offset, best);
    ...
    else if (const auto* addr_of = std::get_if<curlee::parser::AddrOfExpr>(&expr.node))
    {
        if (addr_of->target)
        {
            best = find_expr_at(*addr_of->target, offset, best);
        }
    }
                                              ...
                                              const curlee::parser::Expr* best)
{
                 ...
                 std::get_if<curlee::parser::RuntimePhysReadExpr>(&expr.node))
    {
        if (phys_read->addr)
        {
            best = find_call_expr_at(*phys_read->addr, offset, best);
        }
    }
    else if (const auto* phys_write =
                 std::get_if<curlee::parser::RuntimePhysWriteExpr>(&expr.node))
    {
        if (phys_write->addr)
        {
            best = find_call_expr_at(*phys_write->addr, offset, best);
        }
        if (phys_write->value)
        {
            best = find_call_expr_at(*phys_write->value, offset, best);
    ...
    else if (const auto* addr_of = std::get_if<curlee::parser::AddrOfExpr>(&expr.node))
    {
        if (addr_of->target)
        {
            best = find_call_expr_at(*addr_of->target, offset, best);
        }
    }
    else if (const auto* group = std::get_if<curlee::parser::GroupExpr>(&expr.node))
    {
